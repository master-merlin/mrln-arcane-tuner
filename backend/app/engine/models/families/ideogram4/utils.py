...
# Per-channel shift/scale for the 128-dim patchified latent space, copied
# verbatim from upstream ``src/ideogram4/latent_norm.py`` (LATENT_SHIFT /
# LATENT_SCALE; get_latent_norm()). Each is a length-128 tuple. We materialise
# them as module-level float32 [128] tensors and reshape for broadcast over the
# trailing feature dim of a [B, S, 128] sequence.
...
_LATENT_SHIFT_T = torch.tensor(LATENT_SHIFT, dtype=torch.float32)
_LATENT_SCALE_T = torch.tensor(LATENT_SCALE, dtype=torch.float32)
assert _LATENT_SHIFT_T.shape == (128,) and _LATENT_SCALE_T.shape == (128,)


def normalize_latents(seq: torch.Tensor) -> torch.Tensor:
    """``(seq - shift) / scale`` over the trailing 128-dim. ``seq`` is [B, S, 128]."""
    shift = _LATENT_SHIFT_T.to(device=seq.device, dtype=seq.dtype)
    scale = _LATENT_SCALE_T.to(device=seq.device, dtype=seq.dtype)
    return (seq - shift) / scale


def denormalize_latents(seq: torch.Tensor) -> torch.Tensor:
    """Inverse of :func:`normalize_latents`: ``seq * scale + shift``."""
    shift = _LATENT_SHIFT_T.to(device=seq.device, dtype=seq.dtype)
    scale = _LATENT_SCALE_T.to(device=seq.device, dtype=seq.dtype)
