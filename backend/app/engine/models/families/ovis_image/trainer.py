...
This module implements the Ovis-Image-specific behaviour and carries the
...
class OvisImageTrainer(GenericTrainingPipeline):
    ...
    flow-matching noise schedule. Supports CFG with negative prompts.
    """
    ...
    def transformer(self) -> torch.nn.Module | None:
        ...
        OvisImageDriver stores its primary model on ``self.driver.model``
        (not ``.transformer``), so the base ``_assign_components`` loop would
        set ``self.transformer = None``. This property delegates directly to
        the driver so ``trainer.transformer`` never goes stale (C4).
        """
    ...
    def _pre_cache_text_embeddings(self) -> None:
        ...
        Layout (mirrors krea2 / qwen_image):
        ...
        the sampler runs after the TE is offloaded, so it must serve all
        prompts from ``self.text_cache`` — without this, sampling causes a
        VRAM spike by reloading the text encoder from CPU.
        """
    ...
    def encode_text(
        ...
        """Encode captions through the Ovis Qwen3 text encoder.
        ...
        Returns:
            (text_embeddings ``[B, 256, 2048]``, attention_mask ``[B, 256]``).
        """
    ...
    def _encode_text_direct(
        ...
        pipeline-identical Ovis prompt encoding) and unwraps the
