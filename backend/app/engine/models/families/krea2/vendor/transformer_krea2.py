...
# Source: src/diffusers/models/transformers/transformer_krea2.py
#
# Shimmed for diffusers==0.38.0 compatibility.  Symbols present in 0.38 are
# imported normally; the one symbol missing in 0.38 falls back to a local shim.
# When a future diffusers bump provides the real symbol the try-branch wins
# automatically (no code change needed).
#
# Shim summary (verified against diffusers 0.38.0):
#   FOUND in 0.38 (imported directly):
#     apply_lora_scale, AttentionMixin, AttentionModuleMixin,
#     dispatch_attention_fn, apply_rotary_emb, get_1d_rotary_pos_embed,
#     ConfigMixin, register_to_config, PeftAdapterMixin,
#     Transformer2DModelOutput, ModelMixin
#   MISSING in 0.38 (shimmed below):
#     maybe_adjust_dtype_for_device  — only used for RoPE freqs dtype;
#       MPS needs float32, everything else float64 (the upstream default).
#
...
# ── Shim: maybe_adjust_dtype_for_device ───────────────────────────────────────
# Present in diffusers 0.39+; absent in 0.38.  The function is used only inside
# Krea2RotaryPosEmbed.forward to pick the dtype for RoPE frequency computation.
# On MPS, torch.float64 is unsupported so float32 is substituted; on all other
# devices (CUDA / CPU) float64 is returned unchanged.
try:
    from diffusers.utils.torch_utils import maybe_adjust_dtype_for_device  # 0.39+
except ImportError:

    def maybe_adjust_dtype_for_device(dtype: torch.dtype, device: torch.device) -> torch.dtype:  # type: ignore[misc]
        """Shim for diffusers<0.39: return float32 on MPS, otherwise pass dtype through."""
        if device.type == "mps":
