        ...
        @if (currentPair(); as pair) {
            <div class="relative w-full flex-1 min-h-0 p-4 flex items-center justify-center overflow-hidden"
                ...
                @if (pair.media_type === 'video') {
                    ...
                    <div class="w-full max-w-xl flex flex-col items-center gap-4 px-6" data-testid="detail-audio-player">
                        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="text-text-subtle opacity-40"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg>
                        <audio [src]="getMediaUrl(pair.media_file)" controls class="w-full"></audio>
                    </div>
                } @else {
