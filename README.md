...
mastermerlin/mrln-arcane-tuner:0.6.0-beta        # pinned version (CUDA 12.8 / cu128)
mastermerlin/mrln-arcane-tuner:0.6.0-beta-cu126  # fallback for legacy R560–R565 drivers
```
...
docker build -t mastermerlin/mrln-arcane-tuner:0.6.0-beta -t mastermerlin/mrln-arcane-tuner:latest .
docker push mastermerlin/mrln-arcane-tuner:0.6.0-beta
...
docker build --build-arg CUDA_BASE=12.6.3 --build-arg TORCH_CUDA=cu126 \
    -t mastermerlin/mrln-arcane-tuner:0.6.0-beta-cu126 .
docker push mastermerlin/mrln-arcane-tuner:0.6.0-beta-cu126
```
...
- **GPU:** any NVIDIA Ampere+ GPU, **including Blackwell** (RTX 50xx / RTX PRO
  ...
  `:0.6.0-beta-cu126` tag instead (no Blackwell support). Avoid CUDA 13 in the
