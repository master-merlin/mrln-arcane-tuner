"""PRXTrainer — family-specific trainer for latent PRX.
...
This module implements the PRX-specific behaviour and carries the
...
class PRXTrainer(GenericTrainingPipeline):
    """PRX LoRA trainer.

    ~1.2B cross-attention DiT (16 PRXBlocks, fused QKV/KV projections)
    with a T5Gemma text encoder (hidden 2304, bool masks), Flux-style
    AutoencoderKL, and flow-matching noise schedule. Supports CFG with
    negative prompts.
    """
    ...
    def _setup_family(self) -> None:
        """Initialize PRX-specific loader, driver, and saver."""
        from .loader import PRXLoader  # noqa: PLC0415
        from .driver import PRXDriver  # noqa: PLC0415
        from .saver import PRXSaver  # noqa: PLC0415

        self.loader = PRXLoader(self.device)
        self.driver = PRXDriver(self.definition, self.device)
        self.saver = PRXSaver()

    def _create_sampler(self):
        """Create a PRXSampler if sampling is configured."""
        interval = int(self.config.get("sample_every_n_steps", 0))
        if interval > 0:
            from .sampler import PRXSampler  # noqa: PLC0415

            return PRXSampler(self)
        return None
    ...
    def transformer(self) -> torch.nn.Module | None:
        ...
        PRXDriver stores its primary model on ``self.driver.model`` (not
        ``.transformer``), so the base ``_assign_components`` loop would
        set ``self.transformer = None``. This property delegates directly
        to the driver so ``trainer.transformer`` never goes stale (C4).
        """
    ...
    def encode_text(
        ...
        """Encode captions through the T5Gemma text encoder.
        ...
        Returns:
            (text_embeddings ``[B, 256, 2304]``, BOOL attention_mask
            ``[B, 256]``).
        """
    ...
    def _encode_text_direct(
        ...
        pipeline-identical PRX prompt encoding) and unwraps the
