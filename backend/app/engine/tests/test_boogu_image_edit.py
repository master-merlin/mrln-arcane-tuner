...
class TestEditTeStaysResident:
    def test_offload_text_encoders_is_suppressed(self):
        """Mirror of the qwen_edit pin (GPU UAT 2026-07-14): edit runs encode
        lazily per (caption, control) composite key for the whole run, so the
        shared offload must not move/pop the TE out from under the first
        mid-training cache miss."""
        from app.engine.models.families.boogu_image.trainer_edit import (
            BooguImageEditTrainer,
        )
