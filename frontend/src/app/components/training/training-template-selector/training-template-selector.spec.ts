...
describe('TrainingTemplateSelectorComponent — auto-apply active template on load', () => {
  let svc: { listTrainingTemplates: jasmine.Spy };
  let emitted: Array<{ config: unknown; isDefault: boolean; definitionId?: string; auto?: boolean }>;

  function build(templates: Template[]) {
    svc = { listTrainingTemplates: jasmine.createSpy('list').and.returnValue(of(templates)) };
  ...
  it('applies the synthetic Default once when no real default exists', () => {
    ...
    expect(emitted[0].isDefault).toBeTrue();
    expect(emitted[0].auto).toBeTrue();
  });
