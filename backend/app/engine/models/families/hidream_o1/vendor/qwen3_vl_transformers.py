...
_sage_attn_import_error = None


def get_sage_attention_func():
    global _sage_attn_func, _sage_attn_import_error
    if _sage_attn_func is not None:
        return _sage_attn_func
    if _sage_attn_import_error is not None:
        return None
    try:
        from sageattention import sageattn

        if hasattr(torch, "compiler"):
            sageattn = torch.compiler.disable()(sageattn)
        _sage_attn_func = sageattn
    except Exception as exc:
        _sage_attn_import_error = exc
        _sage_attn_func = None
    return _sage_attn_func


def _attention_kernel_dtype(*tensors):
...
def _module_compute_dtype(module, fallback=torch.float32):
    dtype = getattr(module, "hidream_compute_dtype", None)
    if dtype is not None:
        return dtype

    weight = getattr(module, "weight", None)
    if weight is not None and not _is_float8_dtype(weight.dtype):
        return weight.dtype

    dtype = getattr(module, "weight_comfy_model_dtype", None)
...
def _embedding(module, input_ids, dtype=None):
    dtype = dtype or _module_compute_dtype(module, torch.float32)
    try:
        return module(input_ids, out_dtype=dtype)
    except TypeError:
        return module(input_ids).to(dtype=dtype)
...
from .compat import TransformersKwargs, Unpack, auto_docstring, check_model_inputs  # MRLN-PATCH: adjusted relative import for vendor/ package layout (was ..compat in Saganaki22's hidream_o1 package)
try:
    from transformers.models.qwen3_vl.configuration_qwen3_vl import Qwen3VLConfig, Qwen3VLTextConfig, Qwen3VLVisionConfig
except Exception as exc:
    raise ImportError(
        "HiDream O1 requires a Transformers build with Qwen3-VL support. "
        "Install transformers>=4.57.1 in the ComfyUI environment."
    ) from exc
...
class Qwen3VLVisionPatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        target_dtype = _module_compute_dtype(self.proj, hidden_states.dtype)
...
def repeat_kv_nhd(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    if n_rep == 1:
        return hidden_states
    return repeat_kv(hidden_states.transpose(1, 2), n_rep).transpose(1, 2).contiguous()
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if attention_mask is not None:
        ...
        causal_mask = torch.ones(q_len, k_len, dtype=torch.bool, device=query.device).triu(
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def compute_default_rope_parameters(config=None, device=None, seq_len=None, layer_type=None):
        return ROPE_INIT_FUNCTIONS["default"](config, device=device, seq_len=seq_len, layer_type=layer_type)
...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def fast_pos_embed_interpolate(self, grid_thw, target_device=None, target_dtype=None):
        ...
        pos_device = target_device or _module_device(self.pos_embed, grid_thw.device)
        pos_dtype = target_dtype or _module_compute_dtype(self.pos_embed, torch.float32)
        idx_tensor = torch.tensor(idx_list, dtype=torch.long, device=pos_device)
        weight_tensor = torch.tensor(
            weight_list, dtype=pos_dtype, device=pos_device
        )
        pos_embeds = _embedding(self.pos_embed, idx_tensor, dtype=pos_dtype) * weight_tensor[:, :, None]
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        rotary_pos_emb = self.rot_pos_emb(grid_thw).to(device=hidden_states.device, dtype=hidden_states.dtype)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if inputs_embeds is None:
            inputs_embeds = _embedding(self.embed_tokens, input_ids)
...
class BottleneckPatchEmbed(nn.Module):
    ...
    def initialize_weights(self):
        if self.proj1.weight is not None:
            w1 = self.proj1.weight.data
            nn.init.xavier_uniform_(w1.view([w1.shape[0], -1]))
        if self.proj2.weight is not None:
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, config, hidden_size, frequency_embedding_size=256):
        ...
        if self.mlp[0].weight is not None:
            nn.init.normal_(self.mlp[0].weight, std=0.02)
        if self.mlp[2].weight is not None:
    ...
    def forward(self, t):
        ...
        t_emb = self.mlp(t_freq.to(_module_compute_dtype(self.mlp[0], t_freq.dtype)))
        return t_emb
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        Args:
            ...
            image_grid_thw (`torch.LongTensor` of shape `(num_images, 3)`, *optional*):
                The temporal, height and width of feature shape of each image in LLM.
        """
        pixel_values = pixel_values.to(dtype=_module_compute_dtype(self.visual, pixel_values.dtype))
    ...
    def get_placeholder_mask(
        ...
        if input_ids is None:
            special_image_mask = inputs_embeds == _embedding(
                self.get_input_embeddings(),
                torch.tensor(self.config.image_token_id, dtype=torch.long, device=inputs_embeds.device),
                inputs_embeds.dtype,
            )
            special_image_mask = special_image_mask.all(-1)
            special_video_mask = inputs_embeds == _embedding(
                self.get_input_embeddings(),
    ...
    def _run_decoder_flash(self, inputs_embeds, position_ids, token_types, return_mid_results_layers=None,
        ...
        """Run decoder layers with a two-pass fast attention backend.
        ...
        if attention_backend == "flash":
            ...
            sage_attn_func = get_sage_attention_func()
            if sage_attn_func is None:
                raise RuntimeError("SageAttention is not available. Install a compatible sageattention package.")
        else:
        ...
        def _flash_layer_forward(hidden_states, decoder_layer, cos, sin, idx_ar):
            """Fast attention layer forward compatible with FSDP2.
            ...
            def _custom_fast_attn(hidden_states, position_embeddings, attention_mask=None, **kwargs):
                ...
                attn_dtype = _attention_kernel_dtype(q, k, v)

                def _call_attention(q_in, k_in, v_in, causal):
                    if attention_backend == "flash":
                        ...
                        if k_in.shape[2] != q_in.shape[2]:
                            k_in = repeat_kv_nhd(k_in, attn.num_key_value_groups)
                            v_in = repeat_kv_nhd(v_in, attn.num_key_value_groups)
                        result = sage_attn_func(
                            q_in.to(attn_dtype),
                            k_in.to(attn_dtype),
                            v_in.to(attn_dtype),
                            is_causal=causal,
                ...
                out_ar = _call_attention(q_ar, k_ar, v_ar, causal=True)

                # Pass 2: full (bidirectional) attention on all tokens
                out_full = _call_attention(q, k, v, causal=False)
            ...
            decoder_layer.self_attn.forward = _custom_fast_attn
            try:
    ...
    def _forward_generation(self, input_ids, position_ids, vinputs, timestep, token_types,
        ...
        Args:
            ...
            use_sage_attn: if True, use SageAttention with the same two-pass approach

        Returns:
            Qwen3VLModelOutputWithPast with x_pred field set.
        """
        # 1. Get text token embeddings
        inputs_embeds = _embedding(self.get_input_embeddings(), input_ids)  # [batch, txt_seq_len, hidden]
        ...
        if pixel_values is not None:
            ...
            fake_pv   = torch.zeros(n_patches, patch_dim,
                                    ...
                                    dtype=_module_compute_dtype(pe.proj, inputs_embeds.dtype))
        ...
        if _mem_debug2:
            ...
            print(f"[MEM][rank{_rank2}][_forward_gen] before decoder: alloc={_a:.2f}GB, "
                  f"total_seq_len={total_seq_len}, batch={batch_size}, flash={use_flash_attn}, sage={use_sage_attn}", flush=True)
        ...
        if use_flash_attn:
            ...
            # SageAttention: same two-pass pattern, using sageattention kernels.
            hidden_states, mid_results = self._run_decoder_flash(
                inputs_embeds, position_ids, token_types,
                return_mid_results_layers=return_mid_results_layers,
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if inputs_embeds is None:
            inputs_embeds = _embedding(self.get_input_embeddings(), input_ids)
