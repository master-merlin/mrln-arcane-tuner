"""HunyuanVideo 1.5 per-prompt still previews (Phase 3 — wire
``SamplePromptConfig.num_frames``). ``None`` = run default; ``1`` = still
(one latent frame, ``4n+1`` rule satisfied).
"""
...
def _sampler(config: dict) -> Hv15Sampler:
    pipe = SimpleNamespace(config=config, device=torch.device("cpu"))
    s = object.__new__(Hv15Sampler)
    s.pipeline = pipe
    s.config = config
