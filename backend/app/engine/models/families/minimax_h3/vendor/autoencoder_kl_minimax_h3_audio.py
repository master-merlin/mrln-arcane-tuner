...
# MRLN-PATCH: upstream relative imports (`...`/`..`/`.`) -> absolute
# `diffusers.*`. This file is vendored OUTSIDE the diffusers package tree, so
# upstream's package-relative resolution does not apply here; every symbol
# below was verified present in the installed diffusers 0.39.0, including
# `.vae` (upstream's own `diffusers.models.autoencoders.vae`, not vendored).
