...
# ── Official JoyCaption Beta One prompt templates ────────────────────
# Source: https://github.com/fancyfeast/joy  (app_mm.py)
# Each caption type has 3 templates:
#   [0] = "any" length
#   [1] = numeric word-count  (uses {word_count})
#   [2] = descriptive length  (uses {length})
...
# Maps extra_option param keys → the exact sentence appended to the prompt.
EXTRA_OPTIONS_MAP = {
    ...
    "no_ambiguous_language": "Do NOT use any ambiguous language.",
    "include_nsfw_rating": "Include whether the image is sfw, suggestive, or nsfw.",
    "only_important_elements": "ONLY describe the most important elements of the image.",
    "exclude_artist_name": "If it is a work of art, do not include the artist's name or the title of the work.",
    "identify_orientation": "Identify the image orientation (portrait, landscape, or square) and aspect ratio if obvious.",
    "use_profanity": 'Use vulgar slang and profanity, such as (but not limited to) "fucking," "slut," "cock," etc.',
    "no_euphemisms": "Do NOT use polite euphemisms\u2014lean into blunt, casual phrasing.",
...
def build_joycaption_prompt(params: dict) -> str:
    """Build the full JoyCaption prompt from params, matching the official app logic."""
    caption_type = params.get("caption_type", "Descriptive")
    caption_length = params.get("caption_length", "long")
    name_input = params.get("name_input", "")

    # Select template index:  0=any, 1=word-count, 2=descriptive length
    ...
    # Collect enabled extra options
    extras = []
    for key, sentence in EXTRA_OPTIONS_MAP.items():
...
class JoyCaptionModel(CaptionModel):
    ...
    def generate(self, image: Image.Image, params: dict) -> str:
        ...
        # Build prompt using official logic
        temperature = params.get("temperature", 0.6)
        top_p = params.get("top_p", 0.9)
        max_tokens = params.get("max_tokens", 512)

        # Allow custom prompt override via system_prompt, otherwise use
        # the official build_prompt logic
        if params.get("system_prompt"):
            prompt = params["system_prompt"]
        else:
            prompt = build_joycaption_prompt(params)

        logger.info("joycaption_prompt", prompt=prompt[:200])
