"""GPU smoke scenarios for the LTX-2 + WAN 2.x trainer-hardening work, and
for Krea-2 image LoRA UAT.
...
Scenarios (image):
    krea2_raw   Krea-2 Raw image LoRA, rk32, CFG 4.5  -> Krea-2-Raw GPU UAT
    krea2_turbo Krea-2 Turbo image LoRA (distilled)   -> Krea-2-Turbo GPU UAT
...
# Image-only dataset for image-family smokes (no video keys required).
_DATASETS_IMAGE = [
    {"dataset_name": "Airwolf_images", "num_repeats": 1, "caption_dropout_rate": 0.1},
]
...
def _base_image(definition_id: str) -> dict:
    """Smoke config for image-only LoRA families (no video keys).

    Uses Krea/LTX-class recommended recipe: rank/alpha 32, LR 1e-4,
    CFG guidance_scale 4.5 so the step-0 preview actually shows the LoRA
    (CFG=1 under-shows; mirrors the LTX-2 CFG fix from 0b47bb5).
    Resolution 1024 matches the krea2-raw/turbo definition defaults.
    """
    return {
        "definition_id": definition_id,
        "datasets": _DATASETS_IMAGE,
        "resolutions": [1024],
        "train_batch_size": 1,
        "gradient_accumulation_steps": 1,
        "network_rank": 32,
        ...
        "sample_every_n_steps": 20,
        "sample_prompts": _SAMPLE_PROMPTS,
        "guidance_scale": 4.5,          # CFG so the preview shows the LoRA (not CFG=1)
        "mixed_precision": "bf16",
        "gradient_checkpointing": True,
        "quantization": "none",
        "cache_latents": True,
...
def _scenario(name: str) -> tuple[str, dict]:
    ...
    if name == "krea2_raw":
        return "krea2-raw", _base_image("krea2-raw")
    if name == "krea2_turbo":
        return "krea2-turbo", _base_image("krea2-turbo")
    raise SystemExit(f"unknown scenario {name!r}; see --help")


def main() -> None:
    p = argparse.ArgumentParser(description="LTX-2 + WAN trainer-hardening GPU smokes, and Krea-2 image LoRA UAT")
    p.add_argument(
        "scenario",
        help="wan21_t2v | wan22_t2v | wan21_i2v | ltx2_t2v | ltx2_i2v | krea2_raw | krea2_turbo",
    )
    ...
    mode = cfg.get("video_mode", "image")
    print(f"# scenario: {args.scenario}  ->  lora_name={cfg['lora_name']}")
    print(f"# definition: {definition_id}  mode={mode}  steps={cfg['max_train_steps']}")
    if not args.run:
